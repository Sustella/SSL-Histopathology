...
# sample,patient,node,x_coord,y_coord,tumor,slide,center,split
  ...
  with open(file_path) as f:
    ...
    for example in examples_list:
      ...
      #print(folder_name)
      example_label_dict[sample_name] = class_id
      folder_sample_dict[sample_name] = folder_name
      label_list.append(class_id)
      #print(class_id)
      #break
  labels = set(label_list)
  for label in labels:
    make_dir(os.path.join(dataset_output_dir,label))
  for key, value in example_label_dict.items():
    src = os.path.join(input_image_dir, "patches", folder_sample_dict[key], key + '.'+ image_format)
    #print(src)
    dst = os.path.join(dataset_output_dir, value, key + '.' + image_format)
